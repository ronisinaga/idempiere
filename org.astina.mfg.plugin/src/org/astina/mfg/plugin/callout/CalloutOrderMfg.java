 ...
 */
public class CalloutOrderMfg extends CalloutEngine
{
	/**	Debug Steps			*/
	...
	public String product (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		mTab.setValue("ProductType", mp.getProductType());
		//end
		...
		if (Env.isSOTrx(ctx, WindowNo))
		{
			...
			MOrder mo = new MOrder(ctx, C_Order_ID, null);
			MDocType dt = MDocType.get(ctx, mo.getC_DocTypeTarget_ID());
			
			if(dt.getName().equalsIgnoreCase("Binding offer"))
			{
					...
					&& !(product.isBOM() && product.isVerified() && product.isAutoProduce()))
				{
					...
					{
						//Astina 040223 add mTab.setValue("QtyOrdered", Env.ZERO); and qtyentered

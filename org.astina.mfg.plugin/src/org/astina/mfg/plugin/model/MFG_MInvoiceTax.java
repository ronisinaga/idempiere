 ...
 */
public class MFG_MInvoiceTax extends MInvoiceTax
{
	/**
	 * 
	 */
	 ...
	 */
	public static MFG_MInvoiceTax get (MFG_MInvoiceLine line, int precision, 
		boolean oldTax, String trxName)
	{
		MFG_MInvoiceTax retValue = null;
		if (line == null || line.getC_Invoice_ID() == 0)
			return null;
		int C_Tax_ID = line.getC_Tax_ID();
		boolean isOldTax = oldTax && line.is_ValueChanged(MFG_MInvoiceLine.COLUMNNAME_C_Tax_ID); 
		if (isOldTax)
		{
			Object old = line.get_ValueOld(MFG_MInvoiceLine.COLUMNNAME_C_Tax_ID);
			if (old == null)
				return null;
		...
		//	Create New
		retValue = new MFG_MInvoiceTax(line.getCtx(), 0, trxName);
		retValue.set_TrxName(trxName);
	 ...
	 *	@return existing or new tax
	 */
	public static MFG_MInvoiceTax[] getChildTaxes(MFG_MInvoiceLine line, int precision, 
		boolean oldTax, String trxName)
	{
		List<MFG_MInvoiceTax> invoiceTaxes = new ArrayList<MFG_MInvoiceTax>();
		
		if (line == null || line.getC_Invoice_ID() == 0)
			return invoiceTaxes.toArray(new MFG_MInvoiceTax[0]);
		...
		for(MTax cTax : cTaxes) {
			MFG_MInvoiceTax invoiceTax = new Query(line.getCtx(), Table_Name, "C_Invoice_ID=? AND C_Tax_ID=?", trxName)
			...
			{
				//	Create New
				invoiceTax = new MFG_MInvoiceTax(line.getCtx(), 0, trxName);
				invoiceTax.set_TrxName(trxName);
	...
	/**	Static Logger	*/
	private static CLogger	s_log	= CLogger.getCLogger (MFG_MInvoiceTax.class);
	 ...
	 */
	public MFG_MInvoiceTax (Properties ctx, int ignored, String trxName)
	{
		super(ctx, 0, trxName);
		if (ignored != 0)
	 ...
	 *	@param trxName transaction
	 */
	public MFG_MInvoiceTax (Properties ctx, ResultSet rs, String trxName)
	{
	 ...
	 * @param copy
	 */
	public MFG_MInvoiceTax(MFG_MInvoiceTax copy) 
	{
	 ...
	 * @param copy
	 */
	public MFG_MInvoiceTax(Properties ctx, MFG_MInvoiceTax copy) 
	{
	 ...
	 * @param trxName
	 */
	public MFG_MInvoiceTax(Properties ctx, MFG_MInvoiceTax copy, String trxName) 
	{
		this(ctx, 0, trxName);
		copyPO(copy);

 ...
 *  
 */
public class MFG_MCostDetail extends MCostDetail
{
	/**
	 * 
	 */
	...
	{
		MFG_MCostDetail cd = get (as.getCtx(), "C_OrderLine_ID=? AND Coalesce(M_CostElement_ID,0)="+M_CostElement_ID, 
			C_OrderLine_ID, M_AttributeSetInstance_ID, as.getC_AcctSchema_ID(), trxName);
		//
		if (cd == null)		//	createNew
		{
			cd = new MFG_MCostDetail (as, AD_Org_ID, 
	...
	public static boolean createInvoice (MAcctSchema as, int AD_Org_ID, 
		...
		String Description, String trxName)
	{
		MFG_MCostDetail cd = get (as.getCtx(), "C_InvoiceLine_ID=? AND Coalesce(M_CostElement_ID,0)="+M_CostElement_ID+" AND M_Product_ID="+M_Product_ID, 
	...
	{
		MFG_MCostDetail cd = get (as.getCtx(), "M_InOutLine_ID=? AND Coalesce(M_CostElement_ID,0)="+M_CostElement_ID, 
	...
	public static boolean createInventory (MAcctSchema as, int AD_Org_ID, 
		...
		String Description, String trxName)
	{
		MFG_MCostDetail cd = get (as.getCtx(), "M_InventoryLine_ID=? AND Coalesce(M_CostElement_ID,0)="+M_CostElement_ID, 
		...
		String Description, String trxName)
	{
		...
		MFG_MCostDetail cd = get (as.getCtx(),msget.toString(), 
	...
	public static boolean createProduction (MAcctSchema as, int AD_Org_ID, 
		...
		String Description, String trxName)
	{
		MFG_MCostDetail cd = get (as.getCtx(), "M_ProductionLine_ID=? AND Coalesce(M_CostElement_ID,0)="+M_CostElement_ID, 
	...
	public static boolean createMatchInvoice (MAcctSchema as, int AD_Org_ID, 
			...
			String Description, String trxName)
	{
		MFG_MCostDetail cd = get (as.getCtx(), "M_MatchInv_ID=? AND Coalesce(M_CostElement_ID,0)="+M_CostElement_ID, 
	...
	public static boolean createProjectIssue(MAcctSchema as, int AD_Org_ID, 
		...
		String Description, String trxName)
	{
		MFG_MCostDetail cd = get (as.getCtx(), "C_ProjectIssue_ID=? AND Coalesce(M_CostElement_ID,0)="+M_CostElement_ID, 
	 ...
	 *  @deprecated
	 */
	public static MFG_MCostDetail get (Properties ctx, String whereClause,
		int ID, int M_AttributeSetInstance_ID, String trxName)
	{
		...
		try
		{
			...
			if (rs.next ())
				retValue = new MFG_MCostDetail (ctx, rs, trxName);
		}
		catch (Exception e)
		{
		...
		int ID, int M_AttributeSetInstance_ID, int C_AcctSchema_ID, String trxName)
	{
		...
		MFG_MCostDetail retValue = new Query(ctx,I_M_CostDetail.Table_Name,localWhereClause.toString(),trxName)
	...
	public static boolean processProduct (MProduct product, String trxName)
	{
		...
		int counterOK = 0;
		int counterError = 0;
		List<MFG_MCostDetail> list = new Query(product.getCtx(),I_M_CostDetail.Table_Name,whereClause,trxName)
	...
	/**	Logger	*/
	private static CLogger 	s_log = CLogger.getCLogger (MFG_MCostDetail.class);
	 ...
	 *	@param trxName trx
	 */
	public MFG_MCostDetail (Properties ctx, int M_CostDetail_ID, String trxName)
	{
	 ...
	 *	@param trxName trx
	 */
	public MFG_MCostDetail (Properties ctx, ResultSet rs, String trxName)
	{
	 ...
	 *	@param trxName transaction
	 */
	public MFG_MCostDetail (MAcctSchema as, int AD_Org_ID, 
		int M_Product_ID, int M_AttributeSetInstance_ID,
		...
		int Org_ID, int M_ASI_ID)
	{
		...
		MFG_MCost cost = MFG_MCost.get(product, M_ASI_ID, as, 
		...
		{		
			...
			{								
						...
						&& cost.getCurrentCostPriceLL().signum() == 0)
				{
					...
					if (cost.getCurrentCostPrice().signum() == 0)
					{
						cost.setCurrentCostPrice(MFG_MCost.getSeedCosts(product, M_ASI_ID, 
					...
					|| ce.isLifo())
				{
					...
					MFG_MCostQueue cq = MFG_MCostQueue.get(product, getM_AttributeSetInstance_ID(), 
					...
					MFG_MCostQueue[] cQueue = MFG_MCostQueue.getQueue(product, M_ASI_ID, 
			...
			|| getPP_Cost_Collector_ID() != 0)
		{
			...
			else if (ce.isAveragePO())
			{
				...
				else
				{
					if (isVendorRMA)
					{
						System.out.println("afagagaveragepo");
						cost.setWeightedAverage(amt, qty);
					}
					else
					{
			...
			{
				//bugs bawaan system yoyo
//				if (!isVendorRMA && !adjustment)
//				{
//					if (addition)
//					{
//						//	Real ASI - costing level Org
//						System.out.println("fifo"+qty);
...
//					if (log.isLoggable(Level.FINER)) log.finer("QtyAdjust - FiFo/Lifo - " + cost);
					...
					if (!isVendorRMA && !adjustment) {
	                    if (addition) {
	                        final MFG_MCostQueue cq2 = MFG_MCostQueue.get(product, this.getM_AttributeSetInstance_ID(), as, Org_ID, ce.getM_CostElement_ID(), this.get_TrxName());
	                        try {
	        					if(getM_InventoryLine().getM_Inventory().getC_DocType().getDocSubTypeInv().equals(MDocType.DOCSUBTYPEINV_CostAdjustment)){
	        						adjs = true;
	        						jmladjs = 1;
	                        ...
	                        MFG_MCostQueue.adjustQty(product, M_ASI_ID, as, Org_ID, ce, qty.negate(), this.get_TrxName());
	                    }
	                    MFG_MCostQueue[] cQueue2 = MFG_MCostQueue.getQueue(product, M_ASI_ID, as, Org_ID, ce, this.get_TrxName());
	                    ...
	                    final MFG_MCostQueue[] cQueue2 = MFG_MCostQueue.getQueue(product, M_ASI_ID, as, Org_ID, ce, this.get_TrxName());
	                    if (cQueue2 != null && cQueue2.length > 0) {
	                    	System.out.println("gagaga"+ cQueue2[0].getCurrentCostPrice());
	                        cost.setCurrentCostPrice(cQueue2[0].getCurrentCostPrice());
	                    }
	                   // cost.add(amt, qty);
	                    if (this.log.isLoggable(Level.FINER)) {

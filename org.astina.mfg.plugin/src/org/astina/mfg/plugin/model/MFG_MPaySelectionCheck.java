 ...
 */
public class MFG_MPaySelectionCheck extends MPaySelectionCheck
{
	/**
	 * 
	 */
	 ...
	 */
	public static MFG_MPaySelectionCheck getOfPayment (Properties ctx, int C_Payment_ID, String trxName)
	{
		MFG_MPaySelectionCheck retValue = null;
		...
		try
		{
			...
			while (rs.next ())
			{
				MFG_MPaySelectionCheck psc = new MFG_MPaySelectionCheck (ctx, rs, trxName);
				if (retValue == null)
					retValue = psc;
	 ...
	 *	@return pay selection check for payment or null
	 */
	public static MFG_MPaySelectionCheck createForPayment (Properties ctx, int C_Payment_ID, String trxName)
	{
		if (C_Payment_ID == 0)
			return null;
		MFG_MPayment payment = new MFG_MPayment (ctx, C_Payment_ID, null);
		...
		MFG_MPaySelectionCheck psc = new MFG_MPaySelectionCheck(ps, PaymentRule);
		...
		if (payment.getC_Invoice_ID() != 0)
		{
			...
			MFG_MPaymentAllocate[] pAllocs = MFG_MPaymentAllocate.get(payment);
			if (pAllocs.length != 0) {
				int numInv = 0;
				for (int i = 0; i < pAllocs.length; i++) {
					MFG_MPaymentAllocate pAlloc = pAllocs[i];
	 ...
	 */
	public static MFG_MPaySelectionCheck[] get (int C_PaySelection_ID, String PaymentRule, String trxName)
	{
		if (s_log.isLoggable(Level.FINE)) s_log.fine("C_PaySelection_ID=" + C_PaySelection_ID
			+ ", PaymentRule=" +  PaymentRule);
		ArrayList<MFG_MPaySelectionCheck> list = new ArrayList<MFG_MPaySelectionCheck>();
		...
		try
		{
			...
			while (rs.next())
			{
				MFG_MPaySelectionCheck check = new MFG_MPaySelectionCheck (Env.getCtx(), rs, trxName);
				list.add(check);
			}
		}
		...
		//  convert to Array
		MFG_MPaySelectionCheck[] retValue = new MFG_MPaySelectionCheck[list.size()];
	 ...
	 *  @return array of checks
	 */
	static public MFG_MPaySelectionCheck[] get (int C_PaySelection_ID,
		String PaymentRule, int startDocumentNo, String trxName)
	{
		MFG_MPaySelectionCheck[] checks = get(C_PaySelection_ID, PaymentRule, trxName);
		int docNo = startDocumentNo;
		for (MFG_MPaySelectionCheck check : checks)
		{
	 ...
	 * 	@param batch batch
	 */
	public static void confirmPrint (MFG_MPaySelectionCheck check, MPaymentBatch batch)
	{
		...
		if (trxName == null) {
			...
			trx.setDisplayName(MFG_MPaySelectionCheck.class.getName()+"_confirmPrint");
			check.set_TrxName(trxName);
		}
		try {
			MFG_MPayment payment = new MFG_MPayment(check.getCtx(), check.getC_Payment_ID(), trxName);
			//	Existing Payment
			...
			{
				payment = new MFG_MPayment(check.getCtx(), 0, trxName);
				...
				if (check.getQty() == 1 && psls != null && psls.length == 1)
				{
					...
					//
					//Astina 251222 Remove invoice from payment to put in Payment Allocate
					//payment.setC_Invoice_ID (psl.getC_Invoice_ID());
					
					MFG_MInvoice minv = new MFG_MInvoice(check.getCtx(), psl.getC_Invoice_ID(), trxName);
					
					I_C_PaySelectionLine_Mining mps = POWrapper.create(psl, I_C_PaySelectionLine_Mining.class);
					MInvoicePaySchedule paysched = new MInvoicePaySchedule(Env.getCtx(), mps.getC_InvoicePaySchedule_ID(), trxName);
					String name = paysched.getC_PaySchedule().getC_PaymentTerm().getName();
					payment.setDescription(minv.getDescription()+" - "+name+" - "+minv.getDocumentNo());
					
					MDocType dte = MDocType.get(check.getCtx(), minv.getC_DocType_ID());
					if(dte.getName().equalsIgnoreCase("AP Lease") || dte.getName().equalsIgnoreCase("Interest Lease"))
					{
						String sql = "select distinct C_DocType_ID from C_DocType where name = 'AP Lease Payment' AND AD_Client_ID=? ";
						int doctype = org.compiere.util.DB.getSQLValue(null,sql, Env.getAD_Client_ID(check.getCtx()));
						payment.setC_DocType_ID();
						payment.setC_DocType_ID(doctype);
						payment.setDocumentNo();
						String sql1 = "Select MAX(C_Charge_ID) from C_Charge where name like '%Cash In Banks - Transit' and AD_Client_ID=? ";
						int charge = org.compiere.util.DB.getSQLValue(null,sql1, Env.getAD_Client_ID(check.getCtx()));
						payment.setC_Charge_ID(charge);
					}else
						if(dte.getName().contains("Loan"))
						{
							String sql = "select distinct C_DocType_ID from C_DocType where name like '%Loan%' AND AD_Client_ID=? ";
							int doctype = org.compiere.util.DB.getSQLValue(null,sql, Env.getAD_Client_ID(check.getCtx()));
							payment.setC_DocType_ID();
							payment.setC_DocType_ID(doctype);
							payment.setDocumentNo();
				...
				if (payment.getC_Invoice_ID() == 0)
				{
					MPaySelectionLine[] psld = check.getPaySelectionLines(true);
					MPaySelectionLine psl = psld[0];
					MFG_MPaymentAllocate psla = new MFG_MPaymentAllocate(check.getCtx(), null, trxName);
					psla.setC_Payment_ID(payment.getC_Payment_ID());
					psla.setC_Invoice_ID(psl.getC_Invoice_ID());
					psla.setAmount(check.getPayAmt());
					psla.setDiscountAmt(check.getDiscountAmt());
					psla.setWriteOffAmt(check.getWriteOffAmt());
					psla.setInvoiceAmt(check.getPayAmt());
					psla.setAD_Org_ID(payment.getAD_Org_ID());
					
					I_C_PaySelectionLine_Mining mps = POWrapper.create(psl, I_C_PaySelectionLine_Mining.class);
					I_C_PaymentAllocate_Mining lps = POWrapper.create(psla, I_C_PaymentAllocate_Mining.class);
					lps.setC_InvoicePaySchedule_ID(mps.getC_InvoicePaySchedule_ID());
					psla.saveEx();
	 ...
	 */
	public static int confirmPrint (MFG_MPaySelectionCheck[] checks, MPaymentBatch batch, boolean createDepositBatch)
	{
		boolean localTrx = false;
		String trxName = null;
		...
		if (checks.length > 0)
		{
			...
			try {
				...
				{
					MFG_MPaySelectionCheck check = checks[i];
					if (localTrx)
						check.set_TrxName(trxName);
	...
	//Astina 251222 Custom Confirm Print for Leasing
	public static int confirmPrintAstina (MFG_MPaySelectionCheck[] checks, MPaymentBatch batch, boolean createDepositBatch)
	{
		boolean localTrx = false;
		String trxName = null;
		int lastDocumentNo = 0;
		...
		if (checks.length > 0)
		{
			...
			try {
				...
				if (createDepositBatch)
				{
					depositBatch = new MDepositBatch(ctx, 0, trxName) ;
					depositBatch.setC_BankAccount_ID(c_BankAccount_ID);
					String sql = "select distinct C_DocType_ID from C_DocType where name = 'Batch Payment' AND AD_Client_ID=? ";
					int doctype = org.compiere.util.DB.getSQLValue(null,sql, Env.getAD_Client_ID(ctx));
					depositBatch.setC_DocType_ID(doctype);
	...
	public static int confirmPrint (MFG_MPaySelectionCheck[] checks, MPaymentBatch batch)
	{
		return confirmPrint (checks,batch,false) ;
	} // confirmPrint

	/** Logger								*/
	static private CLogger	s_log = CLogger.getCLogger (MFG_MPaySelectionCheck.class);
	 ...
	 *	@param trxName transaction
	 */
	public MFG_MPaySelectionCheck (Properties ctx, int C_PaySelectionCheck_ID, String trxName)
	{
	 ...
	 *	@param trxName transaction
	 */
	public MFG_MPaySelectionCheck(Properties ctx, ResultSet rs, String trxName)
	{
	 ...
	 *	@param PaymentRule payment rule
	 */
	public MFG_MPaySelectionCheck (MPaySelectionLine line, String PaymentRule)
	{
	 ...
	 *	@param PaymentRule payment rule
	 */
	public MFG_MPaySelectionCheck (MPaySelection ps, String PaymentRule)
	{
	...
	public static boolean deleteGeneratedDraft(Properties ctx, int C_Payment_ID, String trxName)
	{
		...
		MFG_MPaySelectionCheck mpsc = MFG_MPaySelectionCheck.getOfPayment (ctx, C_Payment_ID, trxName);

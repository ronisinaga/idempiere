 ...
 */
public class MFG_MCost extends MCost
{
	/**
	 * 
	 */
		...
		boolean zeroCostsOK, String trxName)
	{
		...
		//	Create/Update Costs
		MFG_MCostDetail.processProduct (product, trxName);
		...
		boolean zeroCostsOK, String trxName)
	{
		...
		try
		{
			System.out.println(product.getAD_Client_ID());
			System.out.println(Org_ID);
			System.out.println( product.getM_Product_ID());
			System.out.println(M_ASI_ID);
			System.out.println(M_CostType_ID);
			System.out.println(as.getC_AcctSchema_ID());
			System.out.println(costingMethod);
			...
			while (rs.next ())
			{
				...
				if (s_log.isLoggable(Level.FINEST)) s_log.finest("CurrentCostPrice=" + currentCostPrice
					...
					+ ", CostingMethod=" + cm
					+ ", Percent=" + percent);
				// yoyo update costprice bom
				//if (currentCostPrice != null && currentCostPrice.signum() != 0)
				if (currentCostPrice.add(currentCostPriceLL) != null && currentCostPrice.add(currentCostPriceLL).signum() != 0) 
				{
					if (cm != null)
					{
	...
	//	System.out.println("dadada"+materialCost);
			...
			|| MCostElement.COSTINGMETHOD_Lifo.equals(costingMethod))
		{
			...
			System.out.println("afafafa"+qty);
			materialCost = MFG_MCostQueue.getCosts(product, M_ASI_ID,
				as, Org_ID, ce, qty, trxName);
			if(materialCost == null) {
				materialCost = calculateTaxFromLines(C_OrderLine_ID,qty);
			}
			//System.out.println("ju"+materialCost);
			return materialCost;
		}

		//System.out.println("jufifo"+materialCost);
		//	Other Costs
	...
	private static BigDecimal calculateTaxFromLines (int orderline, BigDecimal qty)
	{
		...
		MFG_MOrderLine mOrderLine = new MFG_MOrderLine(Env.getCtx(), orderline, null);
		//
...
//			return false;
		...
		MAcctSchema as, int Org_ID, String costingMethod, int C_OrderLine_ID)
	{
		...
		if (!MCostElement.COSTINGMETHOD_StandardCosting.equals(costingMethod))
		{
			...
			MFG_MCost cost = get(product, M_ASI_ID, as, Org_ID, ce.getM_CostElement_ID(), product.get_TrxName());
	...
	public static void create (MClient client)
	{
		...
		if (trxName == null)
		{
			...
			trx.setDisplayName(MFG_MCost.class.getName()+"_create");
		}
	...
	private static void createCostingRecord(MProduct product,int M_ASI_ID, MAcctSchema as, int AD_Org_ID, int M_CostElement_ID) {
		MFG_MCost cost = MFG_MCost.get (product, M_ASI_ID,
	 ...
	 *	@return cost price or null
	 */
	public static MFG_MCost get (MProduct product, int M_AttributeSetInstance_ID,
		MAcctSchema as, int AD_Org_ID, int M_CostElement_ID, String trxName)
	{
		...
		//	New
		if (cost == null)
			cost = new MFG_MCost (product, M_AttributeSetInstance_ID,
	 ...
	 * @return cost or null
	 */
	public static MFG_MCost get (Properties ctx, int AD_Client_ID, int AD_Org_ID, int M_Product_ID,
	...
	/**	Logger	*/
	private static CLogger 	s_log = CLogger.getCLogger (MFG_MCost.class);
	 ...
	 *	@param trxName trx
	 */
	public MFG_MCost (Properties ctx, int ignored, String trxName)
	{
	 ...
	 *	@param trxName trx
	 */
	public MFG_MCost (Properties ctx, ResultSet rs, String trxName)
	{
	 ...
	 *	@param M_CostElement_ID cost element
	 */
	public MFG_MCost (MProduct product, int M_AttributeSetInstance_ID,
		MAcctSchema as, int AD_Org_ID, int M_CostElement_ID)
	{
	...
	public void setCurrentQty(BigDecimal CurrentQty) {
		MCostElement ce = (MCostElement)getM_CostElement();
		//System.out.println(ce.getName()+ CurrentQty);

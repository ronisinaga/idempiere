 ...
 */
public class MFG_MCostQueue extends MCostQueue
{
	/**
	 * 
	 */
	 ...
	 */
	public static MFG_MCostQueue get (MProduct product, int M_AttributeSetInstance_ID,
		MAcctSchema as, int AD_Org_ID, int M_CostElement_ID, String trxName)
	{
		MFG_MCostQueue costQ = null;
		...
		try
		{
			...
			if (rs.next ())
				costQ = new MFG_MCostQueue (product.getCtx(), rs, trxName); 
		}
		catch (Exception e)
		{
		...
		//	New
		if (costQ == null)
			costQ = new MFG_MCostQueue (product, M_AttributeSetInstance_ID,
	 ...
	 *	@return cost queue or null
	 */
	public static MFG_MCostQueue[] getQueue (MProduct product, int M_ASI_ID,
		MAcctSchema as, int Org_ID, MCostElement ce, String trxName)
	{
		ArrayList<MFG_MCostQueue> list = new ArrayList<MFG_MCostQueue>();
		...
		try
		{
			//System.out.println("fsfs"+product.getM_Product_ID()+""+ce.getM_CostElement_ID()+""+M_ASI_ID+""+as.getM_CostType_ID()+""+as.getC_AcctSchema_ID()+""+Org_ID);
			...
			rs = pstmt.executeQuery ();
			while (rs.next ())
				list.add(new MFG_MCostQueue (product.getCtx(), rs, trxName)); 
		}
		catch (Exception e)
		{
		...
		MFG_MCostQueue[] costQ = new MFG_MCostQueue[list.size()];	
		list.toArray(costQ);
		return costQ;
	}	//	getQueue
		...
		String trxName)
	{
		...
		MFG_MCostQueue[] costQ = getQueue(product, M_ASI_ID, 
			as, Org_ID, ce, trxName);
		//System.out.println("afafa"+costQ.length);
		//
		...
		for (int i = 0; i < costQ.length; i++)
		{
			System.out.println("sfafaf"+remainingQty);
			MFG_MCostQueue queue = costQ[i];
			//	Negative Qty i.e. add
			if (remainingQty.signum() <= 0)
			{
		...
		if (lastPrice == null)
		{
			lastPrice = MFG_MCost.getSeedCosts(product, M_ASI_ID, as, Org_ID, 
	...
	/**	Logger	*/
	private static CLogger 	s_log = CLogger.getCLogger (MFG_MCostQueue.class);
	 ...
	 *	@param trxName trx
	 */
	public MFG_MCostQueue (Properties ctx, int ignored, String trxName)
	{
	 ...
	 *	@param trxName trx
	 */
	public MFG_MCostQueue (Properties ctx, ResultSet rs, String trxName)
	{
	 ...
	 *	@param trxName transaction
	 */
	public MFG_MCostQueue (MProduct product, int M_AttributeSetInstance_ID, 
		MAcctSchema as, int AD_Org_ID, int M_CostElement_ID, String trxName)
	{

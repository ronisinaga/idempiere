 ...
 */
public class BankStatementGenPayment extends SvrProcess
{
	...
	private String createPayment (X_I_BankStatement ibs) throws Exception
	{
		...
		//
		MFG_MPayment payment = createPayment (ibs.getC_Invoice_ID(), ibs.getC_BPartner_ID(),
	...
	private String createPayment (MBankStatementLine bsl) throws Exception
	{
		...
		//
		MFG_MPayment payment = createPayment (bsl.getC_Invoice_ID(), bsl.getC_BPartner_ID(),
	 ...
	 *	@return payment
	 */
	private MFG_MPayment createPayment (int C_Invoice_ID, int C_BPartner_ID, 
		...
		String Description, int AD_Org_ID)
	{
		...
		if (C_Invoice_ID != 0)
		{
			...
			if(dte.getName().equalsIgnoreCase("Petty Cash") || dte.getName().equalsIgnoreCase("Expense Claim"))
			{
				payment.setC_Charge_ID(invoice.getC_Charge_ID());
				
				int doctype = DB.getSQLValue(get_TrxName(),
    					"SELECT C_DocType_ID FROM C_DocType WHERE AD_Client_ID=? AND Name=? ", Env.getAD_Client_ID(getCtx()),
    					"Petty Cash Payment");
				MDocType dtp = MDocType.get(getCtx(), doctype);
				payment.setC_DocType_ID(dtp.getC_DocType_ID());

 ...
 */
public class CopyOrderMfg extends SvrProcess
{
	/** Order to Copy				*/
	...
	protected String doIt() throws Exception
	{
		...
		//
		MFG_MOrder from = new MFG_MOrder (getCtx(), p_C_Order_ID, get_TrxName());
		MFG_MOrder newOrder = MFG_MOrder.copyFrom (from, p_DateDoc, 
		...
		{
			MFG_MOrder original = new MFG_MOrder (getCtx(), p_C_Order_ID, get_TrxName());
			original.setDocAction(MFG_MOrder.DOCACTION_Complete);
			if (!original.processIt(MFG_MOrder.DOCACTION_Complete)) {
			...
			original.saveEx();
			original.setDocAction(MFG_MOrder.DOCACTION_Close);
			if (!original.processIt(MFG_MOrder.DOCACTION_Close)) {
				log.warning("Order Process Failed: " + original.getDocumentNo() + " " + original.getProcessMsg());
				throw new IllegalStateException("Order Process Failed: " + original.getDocumentNo() + " " + original.getProcessMsg());

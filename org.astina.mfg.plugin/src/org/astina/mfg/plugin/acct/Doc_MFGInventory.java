 ...
 */
public class Doc_MFGInventory extends Doc
{
	 ...
	 * 	@param trxName trx
	 */
	public Doc_MFGInventory (MAcctSchema as, ResultSet rs, String trxName)
	{
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			if (costAdjustment)
			{
				...
				MFG_MCost cost = MFG_MCost.get(product, asiId, as, 
			...
			{		
				...
				if (dr != null)
				{
					...
					//Astina 210223 override line.getC_Charge_ID();
					MInventoryLine invLine = (MInventoryLine) line.getPO();
					if (isReversal(line)
							&& invLine.getC_Charge_ID() != 0) { //line.getC_Charge_ID()
					...
					if (cr != null)
					{
						...
						//Astina 230223 override line.getC_Charge_ID()
						if (invLine.getC_Charge_ID() != 0)	//	explicit overwrite for charge
					...
					if (line.getM_AttributeSetInstance_ID() == 0 ) 
					{
						...
						if (mas != null && mas.length > 0 )
						{
							...
							for (int j = 0; j < mas.length; j++)
							{
								...
								if (!MFG_MCostDetail.createInventory(as, line.getAD_Org_ID(),

 ...
 *
 */
public class Doc_MFGAllocationHdr extends Doc
{
	/**
	 *  Constructor
	 ...
	 * 	@param trxName trx
	 */
	public Doc_MFGAllocationHdr (MAcctSchema as, ResultSet rs, String trxName)
	{
	...
	private DocLine[] loadLines(MAllocationHdr alloc)
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			DocLine_MFGAllocation docLine = new DocLine_MFGAllocation(line, this);
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		{
			DocLine_MFGAllocation line = (DocLine_MFGAllocation)p_lines[i];
			...
			else
			{
				...
				MAccount acct_liability = new MAccount (getCtx(), 0, getTrxName());
				MDocType dte = MDocType.get(getCtx(), invoice.getC_DocType_ID());
				if(dte.getName().startsWith("Petty Cash") || dte.getName().startsWith("Expense Claim"))
				{
					int payables_ID = 0;
					String sql = null;
					...
					try
					{
						...
						pstmt.setInt (1, invoice.getC_Charge_ID());
						pstmt.setInt (2, as.getC_AcctSchema_ID());
						
						rs = pstmt.executeQuery();
						if (rs.next())
						{
							payables_ID = rs.getInt(1);
							acct_liability =  MAccount.get (as.getCtx(), payables_ID);
						}
					...
					{
						acct_liability = getPaymentAcct(as, line.getC_Payment_ID());
				...
				if (as.isAccrual())
				{
					...
					if(dte.getName().startsWith("Petty Cash") || dte.getName().startsWith("Expense Claim"))
					{
						...
						try
						{
							...
							{
								payables_ID = rs.getInt(1);
								bpAcct =  MAccount.get (as.getCtx(), payables_ID);
							}
		...
		BigDecimal allocationSource)
	{
		...
		Doc_MFGInvoice docInvoice = (Doc_MFGInvoice)Doc.get(as,
		...
		if (as.isCreatePOCommitment() && !invoice.isSOTrx())
		{
			...
			for (int i = 0; i < lines.length; i++)
			{
				Fact factC = Doc_MFGOrder.getCommitmentRelease(as, this,
	...
	private MAccount getPaymentAcct (MAcctSchema as, int C_Payment_ID)
	{
		...
		//Astina 251222 change ACCTTYPE_UnallocatedCash to be ACCTTYPE_C_Receivable
		//int accountType = Doc.ACCTTYPE_UnallocatedCash;
		int accountType = Doc.ACCTTYPE_C_Receivable;
		MPayment payFrg = new MPayment (getCtx(), C_Payment_ID, getTrxName());
		MDocType dteFrg = MDocType.get(getCtx(), payFrg.getC_DocType_ID());
		...
		try
		{
			...
			if (rs.next ())
			{
				...
				//Astina 251222
				MPayment payment1 = new MPayment (getCtx(), C_Payment_ID, getTrxName());
				MDocType dte1 = MDocType.get(getCtx(), payment1.getC_DocType_ID());
				if(dte1.getName().equalsIgnoreCase("Petty Cash Payment"))
				{
					int payables_ID = 0;
					String sql1 = null;
					sql1 = "SELECT CH_Expense_Acct FROM C_Charge_Acct WHERE C_Charge_ID=? AND C_AcctSchema_ID=?";
					PreparedStatement pstmt1 = null;
					ResultSet rs1 = null;
					try
					{
						pstmt1 = DB.prepareStatement(sql1, null);
						pstmt1.setInt (1, payment1.getC_Charge_ID());
						pstmt1.setInt (2, as.getC_AcctSchema_ID());
						
						rs1 = pstmt1.executeQuery();
						if (rs1.next())
						{
							payables_ID = rs1.getInt(1);
							return MAccount.get (as.getCtx(), payables_ID);
						}
					}
					catch (SQLException e)
					{
				...
				{
					//Astina 251222 change ACCTTYPE_PaymentSelect to ACCTTYPE_V_Liability
					//accountType = Doc.ACCTTYPE_PaymentSelect;
					accountType = Doc.ACCTTYPE_V_Liability;
				}
				//	Prepayment
				if ("Y".equals(rs.getString(4)))		//	Prepayment
				{
					...
					else
					{
						...
						MDocType dte = MDocType.get(getCtx(), payment.getC_DocType_ID());
						if(dte.getName().startsWith("Advance Employee"))
						{
							...
							{
								pstmt1 = DB.prepareStatement(sql1, null);
								pstmt1.setInt (1, payment.getC_Charge_ID());
								pstmt1.setInt (2, as.getC_AcctSchema_ID());

 ...
 */
public class Doc_MFGOrder extends Doc
{
	/**
	 *  Constructor
	 ...
	 * 	@param trxName trx
	 */
	public Doc_MFGOrder (MAcctSchema as, ResultSet rs, String trxName)
	{
		...
		BigDecimal Qty, int C_InvoiceLine_ID, BigDecimal multiplier)
	{
		...
		DocLine[] commitments = Doc_MFGOrder.getCommitments(doc, Qty,
				C_InvoiceLine_ID);
	...
	{
		Fact fact = new Fact(doc, as, Fact.POST_Commitment);
		DocLine[] commitments = Doc_MFGOrder.getCommitmentsSales(doc, Qty,
				M_InOutLine_ID);
		...
		if (offset == null)
		{
			...
			s_log.log(Level.SEVERE, "@NotFound@ @CommitmentOffsetSales_Acct@");
			return null;
		}
		fact.createLine (null, offset,

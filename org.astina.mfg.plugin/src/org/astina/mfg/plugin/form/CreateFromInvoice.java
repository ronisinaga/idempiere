...
public abstract class CreateFromInvoice extends CreateFrom
{
	...
	protected ArrayList<KeyNamePair> loadShipmentData (int C_BPartner_ID)
	{
		...
		//
		StringBuffer sql = new StringBuffer("SELECT distinct s.M_InOut_ID, ").append(display).append(", co.C_Order_ID")
			.append(" FROM M_InOut s ");
			sql.append(" JOIN M_InOutLine mil ON mil.M_InOut_ID=s.M_InOut_ID"
				+ " JOIN C_OrderLine col ON col.C_OrderLine_ID = mil.C_OrderLine_ID"
				+ " JOIN C_Order co ON co.C_Order_ID = col.C_Order_ID");
			sql.append(" WHERE s.C_BPartner_ID=? AND s.IsSOTrx=? AND s.DocStatus IN ('CL','CO')"
			+ " AND s.M_InOut_ID IN "
			...
			if(!isSOTrx)
				sql.append(" LEFT OUTER JOIN M_MatchInv mi ON (sl.M_InOutLine_ID=mi.M_InOutLine_ID) "
					...
					+ " WHERE s2.C_BPartner_ID=? AND s2.IsSOTrx=? AND s2.DocStatus IN ('CL','CO') AND COALESCE(sl.MovementQty,0)>0"
			...
			sql.append(") ORDER BY co.C_Order_ID");
	...
	protected Vector<Vector<Object>> getShipmentData(int M_InOut_ID)
	{
		...
		sql.append(" l.QtyEntered/l.MovementQty,"
			...
			+ " l.M_Product_ID, p.Name, p.value, l.description, l.M_InOutLine_ID, l.Line,"        //  5..10
			+ " l.C_OrderLine_ID " //  11
			+ " FROM M_InOutLine l "
			);
		...
		sql.append(" LEFT OUTER JOIN M_Product_PO po ON (l.M_Product_ID = po.M_Product_ID AND io.C_BPartner_ID = po.C_BPartner_ID)")
			...
			.append("GROUP BY l.MovementQty, l.QtyEntered/l.MovementQty, "
				...
				+ "l.M_Product_ID, p.Name, p.value, l.M_InOutLine_ID, l.Line, l.C_OrderLine_ID ");
		if(!isSOTrx)
		...
		try
		{
			...
			while (rs.next())
			{
				...
				line.add(rs.getString(8));				// 5-line description
...
//	    MRMA m_rma = new MRMA(Env.getCtx(), M_RMA_ID, null);
	    ...
	    try
	    {
	        ...
	        while (rs.next())
            {
	            ...
	            line.add(null); //5-line description
	            line.add(null); //6-Order
	...
	protected void configureMiniTable (IMiniTable miniTable)
	{
		...
		miniTable.setColumnClass(5, String.class, true);        //  5-Line Description
		miniTable.setColumnClass(6, String.class, true);        //  5-Order
		miniTable.setColumnClass(7, String.class, true);        //  6-Ship
		miniTable.setColumnClass(8, String.class, true);        //  7-Invoice
		//  Table UI
		miniTable.autoSize();
	}
...
//						}
	...
	protected Vector<String> getOISColumnNames()
	{
	    ...
	    columnNames.add(Msg.getElement(Env.getCtx(), "Description", isSOTrx));

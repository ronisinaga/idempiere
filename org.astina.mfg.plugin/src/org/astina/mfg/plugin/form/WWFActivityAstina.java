...
import org.adempiere.webui.apps.wf.WWFActivity;
...
public class WWFActivityAstina extends WWFActivity implements EventListener<Event>
{
	/**
	 * 
	 */
	...
	/**	Logger			*/
	private static final CLogger log = CLogger.getCLogger(WWFActivityAstina.class);
	...
	private final static String HISTORY_DIV_START_TAG = "<div style='overflow-y: auto; height: 100px; border: 1px solid #7F9DB9;'>";
	public WWFActivityAstina()
	{
		super();
	...
	public int loadActivities()
	{
		...
		Iterator<MWFActivity> it = new Query(Env.getCtx(), MWFActivity.Table_Name, MWFActivity.getWhereUserPendingActivities(), null)
				.setApplyAccessFilter(true, false)
				.setParameters(AD_User_ID, AD_User_ID, AD_User_ID, AD_User_ID, AD_User_ID, AD_Client_ID)
				.setOrderBy("AD_WF_Activity.Priority DESC, AD_WF_Activity.Created Desc")
				.iterate();
		...
		while (it.hasNext()) {
			...
			if(activity.getSummary() != null)
			{
				...
				if(activity.getSummary() != null)
				{				
					//Astina 251222 add date create and Business Unit
					PO po = activity.getPO();
					
					sb.append(activity.getSummary()).append("| ").append(po.get_Value(po.get_ColumnIndex("Created")).toString().substring(0, 10)).append(" ");
					int index = po.get_ColumnIndex("PriorityRule");
					if (index != -1)
					{
						String pr = (String)po.get_Value(index);
						if (!pr.isEmpty())
						{
							MRefList element = MRefList.get(Env.getCtx(), 154, pr, null);
							if (element != null)
					...
					if(tablename.equalsIgnoreCase("HR_PKB"))
					{
						int index2 = po.get_ColumnIndex("HR_Job_ID");
						if (index2 != -1)
						{
							Integer job = (Integer)po.get_Value(index2);
							if (job != null)
							{
								String sql1 = "SELECT name from HR_Job where HR_Job_ID=?";
								String name_pos = org.compiere.util.DB.getSQLValueString(null,sql1, job.intValue());
								
								if (!name_pos.isEmpty() || !name_pos.isBlank())
								{
									sb.append("| ");
									sb.append(name_pos.toString()).append(" ");
								}
							}
						}
						
						if(po.get_Value(po.get_ColumnIndex("PKBReason")) != null)
						{
							String PKBReason = MRefList.getListName(Env.getCtx(), 1000175, po.get_Value(po.get_ColumnIndex("PKBReason")).toString()); //1000175 Reference ID PKB Reason
							sb.append("| ").append(PKBReason.toString()).append(" ");
						}
						
						int index3 = po.get_ColumnIndex("PKBSource");
						if (index3 != -1)
						{
							if(po.get_Value(po.get_ColumnIndex("PKBSource")) != null)
							{
								String PKBSource = MRefList.getListName(Env.getCtx(), 1000176, po.get_Value(po.get_ColumnIndex("PKBSource")).toString()); //1000176 Reference ID PKB Source
								sb.append("| ").append(PKBSource.toString());
							}
						}
					}
	...
	public void display (int index)
	{
		...
		if (MWFNode.ACTION_UserChoice.equals(node.getAction()))
		{
			...
			if (m_column != null && m_column.get_ID() != 0)
			{
				...
				{
					//Astina 251222
					fAnswerList.appendItem(null, null);
	...
	public void onOK()
	{
		...
		try {
			...
			else if (MWFNode.ACTION_UserChoice.equals(node.getAction()))
			{
				...
				if (dt == DisplayType.YesNo || DisplayType.isList(dt)) //dt == DisplayType.List)
				{
					ListItem li = fAnswerList.getSelectedItem();
					String a = li.getValue();
					if(a != null) 
					{
					...
					int tableID=0;
					tableID = m_activity.getPO().get_Table_ID();
					if(tableID == 702 || tableID == 259 || tableID == 1000056)
					{
						if(textMsg != null)
						{
							String sDate = new java.text.SimpleDateFormat("MM/dd/yyyy kk:mm").format(System.currentTimeMillis());
							String x = MUser.getNameOfUser(AD_User_ID)+" "+sDate+" IsApproved="+value+ " "+textMsg+"<br/>";
							if(m_activity.getPO().get_Value("ApprovalNote") == null)
							{
								m_activity.getPO().set_ValueNoCheck("ApprovalNote", x);
							}else
							{
								m_activity.getPO().set_ValueNoCheck("ApprovalNote", m_activity.getPO().get_Value("ApprovalNote")+x);
							}
						}
					}
				}

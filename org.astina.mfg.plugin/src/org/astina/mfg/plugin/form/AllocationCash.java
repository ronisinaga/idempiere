...
public class AllocationCash
{
	public DecimalFormat format = DisplayType.getNumberFormat(DisplayType.Amount);

	/**	Logger			*/
	public static final CLogger log = CLogger.getCLogger(AllocationCash.class);
...
//	private int			i_multiplier = 10;
	...
	{		
		...
		if (!m_bpartnerCheck.contains(key))
		{
			new Thread()
			{
				public void run()
				{
					MFG_MPayment.setIsAllocated (Env.getCtx(), m_C_BPartner_ID, null);
					MFG_MInvoice.setIsPaid (Env.getCtx(), m_C_BPartner_ID, null);
				}
			}.start();
	...
	//{		
	//	return MFG_MPayment.getUnAllocatedPrepaymentData(m_C_BPartner_ID, m_C_Currency_ID, isMultiCurrency, date, m_AD_Org_ID, trxName);
	//}
	...
	{		
		...
		StringBuilder sql = new StringBuilder("SELECT p.DateTrx,p.DocumentNo,p.C_Payment_ID,"  //  1..3
			...
			//+ " AND p.C_Charge_ID IS NULL"		//	Prepayments OK  //Astina Comment
			+ " AND p.C_BPartner_ID=?");                   		//      #5
		if (!isMultiCurrency)
			sql.append(" AND p.C_Currency_ID=?");				//      #6
		if (m_AD_Org_ID != 0 )
			sql.append(" AND p.AD_Org_ID=" + m_AD_Org_ID);
		sql.append(" ORDER BY p.DateTrx Desc,p.DocumentNo");

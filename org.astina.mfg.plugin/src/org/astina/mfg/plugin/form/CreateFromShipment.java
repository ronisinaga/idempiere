...
public abstract class CreateFromShipment extends CreateFrom 
{
	...
	protected Vector<Vector<Object>> getOrderDataItem (int C_Order_ID, boolean forInvoice)
	{
		/**
		 *  Selected        - 0
		 *  Qty             - 1
		...
		StringBuilder sql = new StringBuilder("SELECT "
				...
				+ "CASE WHEN l.QtyOrdered=0 THEN 0 ELSE l.QtyEntered/l.QtyOrdered END,"	//	2
				+ " l.C_UOM_ID,COALESCE(uom.UOMSymbol,uom.Name),"			//	3..4
				+ " pLoc.M_Locator_ID, loc.Value, " // 5..6
				+ " COALESCE(l.M_Product_ID,0),COALESCE(p.Name,c.Name), " //	7..8
				+ " p.value, " // 9
				+ " l.description, " // 10
				+ " l.C_OrderLine_ID,l.Line "	//	11..12
				+ "FROM C_OrderLine l"
				+ " JOIN C_Order co on (l.C_Order_ID=co.C_Order_ID)"
				+ " LEFT OUTER JOIN M_Product_PO po ON (l.M_Product_ID = po.M_Product_ID AND l.C_BPartner_ID = po.C_BPartner_ID) "
				+ " LEFT OUTER JOIN M_MatchPO m ON (l.C_OrderLine_ID=m.C_OrderLine_ID AND ");
		sql.append(forInvoice ? "m.C_InvoiceLine_ID" : "m.M_InOutLine_ID");
		sql.append(" IS NOT NULL)")
		.append(" LEFT OUTER JOIN M_Product p ON (l.M_Product_ID=p.M_Product_ID)"
				+ " LEFT OUTER JOIN M_ProductLocator pLoc on (p.M_Product_ID=pLoc.M_Product_ID AND co.AD_Org_ID = pLoc.AD_Org_ID)"
				+ " LEFT OUTER JOIN M_Locator loc on (pLoc.M_Locator_ID=loc.M_Locator_ID)"
				+ " LEFT OUTER JOIN C_Charge c ON (l.C_Charge_ID=c.C_Charge_ID)");
		if (Env.isBaseLanguage(Env.getCtx(), "C_UOM"))
			sql.append(" LEFT OUTER JOIN C_UOM uom ON (l.C_UOM_ID=uom.C_UOM_ID)");
		else
			...
			.append(Env.getAD_Language(Env.getCtx())).append("')");
		//
		sql.append(" WHERE co.C_Order_ID=? "//AND l.ProductType=? "			//	#1
				+ "GROUP BY l.QtyOrdered,CASE WHEN l.QtyOrdered=0 THEN 0 ELSE l.QtyEntered/l.QtyOrdered END, "
				+ "l.C_UOM_ID,COALESCE(uom.UOMSymbol,uom.Name), pLoc.M_Locator_ID, loc.Value, p.value, "
				+ "l.M_Product_ID,COALESCE(p.Name,c.Name), l.Line,l.C_OrderLine_ID "
				+ "ORDER BY l.Line");
		//
		if (log.isLoggable(Level.FINER)) log.finer(sql.toString());
	...
	protected Vector<Vector<Object>> getReceiptData (int M_InOut_ID, int AD_Org_ID)
	{
		/**
		 *  Selected        - 0
		 *  Line			- 1
		 ...
		 *  Description     - 7
		 */
		//int MP_Requisition_ID = ((Integer) getGridTab().getValue("MP_Requisition_ID")).intValue();
		if (log.isLoggable(Level.CONFIG)) log.config("M_InOut_ID=" + M_InOut_ID);
		
		Vector<Vector<Object>> data = new Vector<Vector<Object>>();
		StringBuilder sql = new StringBuilder("SELECT"
				+ " l.Line," //1
				+ " COALESCE(l.M_Product_ID,0), po.value," //	2..3
				+ " po.name," //	4
				+ " l.QtyEntered,"	//	5
				+ " l.C_UOM_ID, COALESCE(uom.UOMSymbol,uom.Name),"			//	6..7
				+ " l.M_Locator_ID, loc.Value,"			//	8..9
				+ " l.Description, l.M_InOutLine_ID ");	//	10..11
				sql = sql.append( " FROM M_InOutLine l"
						+ " JOIN M_InOut m ON l.M_InOut_ID = m.M_InOut_ID "
						+ " JOIN M_Product po ON l.M_Product_ID = po.M_Product_ID "
					    + " JOIN C_UOM uom ON l.C_UOM_ID=uom.C_UOM_ID "
					    + " JOIN M_Locator loc ON l.M_Locator_ID=loc.M_Locator_ID ");
				sql.append(" WHERE m.M_InOut_ID=? and m.AD_Org_ID=? ");
				sql.append(" and not exists (select cil.m_inoutline_id from c_invoiceline cil " + 
						" join c_invoice ci on ci.c_invoice_id=cil.c_invoice_id " + 
						" where ci.docstatus not in ('RE', 'VO') and cil.m_inoutline_id = l.m_inoutline_id) ");
				sql = sql.append(" ORDER BY l.Line");
		...
		try
		{
			...
			while (rs.next())
			{
				Vector<Object> line = new Vector<Object>();
				line.add(Boolean.FALSE);           //  0-Selection
				line.add(rs.getBigDecimal(5));  // 1-Qty Entered
				line.add(rs.getString(1)); // 1-Line
				KeyNamePair pp = new KeyNamePair(rs.getInt(2), rs.getString(3).trim());
				line.add(pp);                           //  2-Product
				line.add(rs.getString(4)); // 3-product name
				line.add(rs.getBigDecimal(5));  // 4-Qty enter
				pp = new KeyNamePair(rs.getInt(6), rs.getString(7));
				line.add(pp);                           //  5-UOM
				pp = new KeyNamePair(rs.getInt(8), rs.getString(9));
				line.add(pp);   //6-Locator
				line.add(rs.getString(10)); // 7-Desc
				line.add(line.add(new KeyNamePair(rs.getInt(11), "."))); // 8-Line ID
				data.add(line);
			}
		}
		catch (SQLException e)
	...
	protected void configureMiniTable (IMiniTable miniTable)
	{
		...
		miniTable.setColumnClass(5, String.class, true); //  ProductNo
		miniTable.setColumnClass(6, String.class, true); //  line description
		...
		miniTable.setColumnClass(10, BigDecimal.class, true);      //  Qty
	...
	protected void configureMiniTableReturn (IMiniTable miniTable)
	{
		miniTable.setColumnClass(0, Boolean.class, false);     //  Selection
		miniTable.setColumnClass(1, BigDecimal.class, false);      //  Qty
		miniTable.setColumnClass(2, String.class, true);     //  Line No
		...
		miniTable.setColumnClass(6, String.class, true);          //  UOM
		miniTable.setColumnClass(7, String.class, true);  //  Locator 
		miniTable.setColumnClass(8, String.class, true);   //  Desc
		miniTable.setColumnClass(9, String.class, true);   //  Line ID
		...
		if(inout.getDocStatus().equalsIgnoreCase("CO"))
		{
			for (int j = 0; j < miniTable.getRowCount(); j++)
			{
				if (((Boolean)miniTable.getValueAt(j, 0)).booleanValue()) {
					...
					BigDecimal QtyChek = (BigDecimal) miniTable.getValueAt(j, 5); // Qty
					if(QtyEntered.compareTo(QtyChek)>0)
					{
						String msg = "Quantity Entered Over Quantity Receipt. Product=" + miniTable.getValueAt(j, 5);
					...
					{
						String msg = "Quantity Entered must greater than 0. Product=" + miniTable.getValueAt(j, 5);
						throw new AdempiereException(msg);
						//return false;
					}
				}
			...
			//Create Header
			MInOut split = new MInOut (inout, inout.getC_DocType_ID(), inout.getMovementDate());
			StringBuilder msgd = new StringBuilder("Splitted from ").append(inout.getDocumentNo());
			split.addDescription(msgd.toString());
			split.setRef_InOut_ID(inout.getM_InOut_ID());
			split.setDocStatus(inout.getDocStatus());
			split.setDocAction(inout.getDocAction());
			split.setProcessed(true);
			split.setProcessedOn(inout.getProcessedOn());
			split.setPosted(true);
			split.setIsApproved(true);
			split.setCreatePackage(inout.getCreatePackage());
			if(inout.getC_Activity_ID()>0)
				split.setC_Activity_ID(inout.getC_Activity_ID());
			
			if(inout.getUser1_ID()>0)
				split.setUser1_ID(inout.getUser1_ID());
			...
			for (int j = 0; j < miniTable.getRowCount(); j++)
			{
				if (((Boolean)miniTable.getValueAt(j, 0)).booleanValue()) 
				{
					// variable values
					BigDecimal QtyEntered = (BigDecimal) miniTable.getValueAt(j, 1); // Qty
					BigDecimal Qtyorigin = (BigDecimal) miniTable.getValueAt(j, 5); // Qty
					KeyNamePair pp = (KeyNamePair) miniTable.getValueAt(j, 9); // Line ID
					int M_InOutLine_ID = pp.getKey();
					MInOutLine splitMil = new MInOutLine(Env.getCtx(), M_InOutLine_ID, trxName);
					if(Qtyorigin.compareTo(QtyEntered)>0)
					{
						//Update qty Original Line
						BigDecimal newQty = Qtyorigin.subtract(QtyEntered);
						splitMil.setQty(newQty);
						splitMil.saveEx();
						
						//Insert New Receipt
						MInOutLine newLine = new MInOutLine(Env.getCtx(), null, trxName);
						MInOutLine.copyValues(splitMil, newLine);
						newLine.setM_InOut_ID(split.get_ID());
						newLine.setQty(QtyEntered);
						StringBuilder description = new StringBuilder().append(split.getDocumentNo());
						description.append(" #").append(splitMil.getLine());
						newLine.saveEx();
						
						if (newLine.getDescription() != null)
							description.append(" (").append(newLine.getDescription()).append(")");
						else if (split.getDescription() != null && split.getDescription().length() > 0)
							description.append(" (").append(split.getDescription()).append(")");
						
						//Update Journal Original Line and new Journal Line
						MFG_MOrderLine mol = new MFG_MOrderLine(Env.getCtx(), splitMil.getC_OrderLine_ID(), trxName);
						String sql = "SELECT fact_Acct_ID FROM fact_Acct "
								+ "WHERE AD_Table_ID = 319 and Line_ID=? AND Record_ID =?";
						...
						try
						{
							...
							while (rs.next())
							{
								MFactAcct act = new MFactAcct(Env.getCtx(), rs.getInt(1), trxName);
								BigDecimal newAmount = newQty.multiply(mol.getPriceActual());
								if(act.getQty().compareTo(Env.ZERO)>0)
								{
									act.setQty(newQty);
									act.setAmtAcctDr(newAmount);
									act.setAmtSourceDr(newAmount);
									
									act.saveEx();
									
									MFactAcct actNew = new MFactAcct(Env.getCtx(), null, trxName);
									MFactAcct.copyValues(act, actNew);
									actNew.setQty(QtyEntered);
									BigDecimal actNewAmount = QtyEntered.multiply(mol.getPriceActual());
									
									actNew.setAmtAcctDr(actNewAmount);
									actNew.setAmtSourceDr(actNewAmount);
									actNew.setRecord_ID(split.get_ID());
									actNew.setLine_ID(newLine.get_ID());
									actNew.setDescription(description.toString());
									actNew.saveEx();
								}else
								{
									act.setQty(newQty.negate());
									act.setAmtAcctCr(newAmount);
									act.setAmtSourceCr(newAmount);
									act.saveEx();
									
									MFactAcct actNew = new MFactAcct(Env.getCtx(), null, trxName);
									MFactAcct.copyValues(act, actNew);
									actNew.setQty(QtyEntered.negate());
									BigDecimal actNewAmount = QtyEntered.multiply(mol.getPriceActual());
									
									actNew.setAmtAcctCr(actNewAmount);
									actNew.setAmtSourceCr(actNewAmount);
									actNew.setRecord_ID(split.get_ID());
									actNew.setLine_ID(newLine.get_ID());
									actNew.setDescription(description.toString());
									actNew.saveEx();
					...
					{
						splitMil.setM_InOut_ID(split.get_ID());
						splitMil.saveEx();
						
						StringBuilder description = new StringBuilder().append(split.getDocumentNo());
						description.append(" #").append(splitMil.getLine());
						
						if (splitMil.getDescription() != null)
							description.append(" (").append(splitMil.getDescription()).append(")");
						else if (split.getDescription() != null && split.getDescription().length() > 0)
							description.append(" (").append(split.getDescription()).append(")");		
									
						DB.executeUpdateEx("Update fact_acct set Record_ID = " + split.getM_InOut_ID() + ", Description = "+"'" + description.toString() + "'"+" WHERE AD_Table_ID = 319 and Line_ID=" + M_InOutLine_ID + " AND Record_ID = "+ M_InOut_ID, trxName);
					}
				}
			}
			
			Dialog.warn(1, "GR No -> "+split.getDocumentNo());
		}else
		{
			if (M_Locator_ID == 0) {
				return false;
			...
			for (int j = 0; j < miniTable.getRowCount(); j++)
			{
				if (((Boolean)miniTable.getValueAt(j, 0)).booleanValue()) {
					// variable values
					BigDecimal QtyEntered = (BigDecimal) miniTable.getValueAt(j, 1); // Qty
					BigDecimal QtyChek = (BigDecimal) miniTable.getValueAt(j, 10); // Qty
					if(QtyEntered.compareTo(QtyChek)>0)
					{
						String msg = "Quantity Entered Over Quantity Outstanding Order. Product=" + miniTable.getValueAt(j, 5);
						throw new AdempiereException(msg);
						//return false;
					}
	...
	protected Vector<String> getOISColumnNames()
	{
	    ...
	    columnNames.add(Msg.getElement(Env.getCtx(), "Name", false));
	...
	protected Vector<String> getReturnColumnNames()
	{
		//  Header Info
	    Vector<String> columnNames = new Vector<String>(9);
	    columnNames.add(Msg.getMsg(Env.getCtx(), "Select"));
	    columnNames.add(Msg.translate(Env.getCtx(), "QtyReturn"));
	    ...
	    columnNames.add(Msg.getElement(Env.getCtx(), "M_InOutLine_ID"));
	...
	protected Vector<Vector<Object>> getOrderDataItem (int C_Order_ID, boolean forInvoice, int M_Locator_ID)
	{
		defaultLocator_ID = M_Locator_ID;
		return getOrderDataItem (C_Order_ID, forInvoice);
	}
